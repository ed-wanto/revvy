...
点按截屏面板上的 **×** 会将其收起为一个按钮，再次点按该按钮即可展开。要完全隐藏面板，请使用菜单栏菜单、设置或快捷键。
...
- **快捷键** — 录制全局快捷键（点按后按下按键；<kbd>Esc</kbd> 取消，<kbd>⌫</kbd> 清除）、显示或隐藏浮动面板、恢复默认。如果其他 App 已占用某个组合键，Revvy 会发出警告
