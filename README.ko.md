...
- 모든 앱 위에 떠 있는 작은 캡처 패널, 버튼 하나 크기로 접기 가능, 크기(4단계)와 색상 변경 가능
- 실행하면 캡처 패널만 나타나고, 캡처하면 편집 화면이 열림
...
1. Revvy를 엽니다. 화면 오른쪽 위 근처에 캡처 패널이 나타납니다. 실행할 때는 이 패널(과 메뉴 막대 아이콘)만 나타나며, 편집 화면은 캡처할 때 열립니다.
...
캡처하면(이미지를 붙여넣거나 드롭할 때도) 편집 화면이 열립니다. 캡처 패널의 **Revvy 열기** 버튼, 메뉴 막대 메뉴, Dock 아이콘으로도 열 수 있습니다.
...
캡처 패널 오른쪽 끝의 **»**를 누르면 버튼 하나 크기로 접히고, **«**를 누르면 다시 펼쳐집니다. 패널을 오른쪽 클릭하면 크기(작게·중간·크게·아주 크게)와 색상을 바꿀 수 있으며, 설정에서도 바꿀 수 있습니다. 패널을 완전히 숨기려면 메뉴 막대 메뉴, 설정 또는 단축키를 사용하세요. 패널을 숨긴 동안에는 실행할 때 편집 화면이 열립니다.
...
**모두 지우기** — 캡처 패널의 지우개 버튼을 누르거나, 메뉴 막대 메뉴 또는 눈금자·안내선의 오른쪽 클릭 메뉴에서 **모든 눈금자와 안내선 지우기**를 선택하면 모든 눈금자와 안내선을 한 번에 지웁니다. 전역 단축키는 설정에서 지정할 수 있습니다.
...
- **플로팅 패널** — 캡처 패널 보기/가리기, 패널 크기(작게·중간·크게·아주 크게)와 색상
- **단축키** — 전역 단축키 기록(클릭한 뒤 키를 누름, <kbd>Esc</kbd> 취소, <kbd>⌫</kbd> 지우기), 기본값으로 복원. 다른 앱이 이미 사용 중인 조합이면 경고합니다
